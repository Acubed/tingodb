	...
	this._tq = new wqueue(100, function (cb) {
		(function (cb) {
			...
			safe.run(function (cb) {
				var size = Object.keys(self._store).length;
				if (deleted > size) {
				...
				// update indexes
				var values = Object.keys(self._store).map(function(k){ return self._store[k]; });
				async.forEachSeries(values, function (rec, cb) {
...
tcoll.prototype._compact = function (cb) {
	...
	fs.open(filename, 'w+', safe.sure(cb, function (fd) {
		...
		var wpos = 0;
		var store = {};
		async.forEachSeries(Object.keys(self._store), function (k, cb) {
...
tcoll.prototype.createIndex = tcoll.prototype.ensureIndex = function (obj, options, cb) {
	...
	if (options._tiarr===undefined && self._tdb._gopts.searchInArray)
		options._tiarr = true;
	...
	if (self._tq._tc==-1) {
		...
		this._tq.add(function (cb) {
			var range = Object.keys(self._store).map(function(k){return self._store[k];});
...
tcoll.prototype.indexes = function (cb) {
	var self = this;
	this._tq.add(function (cb) {
		cb(null, Object.keys(self._idx).map(function(k){return self._idx[k];}));
	},false,cb);
}
...
tcoll.prototype.insert = function (docs, opts, cb ) {
	...
	opts = opts || {};
	if (opts.w>0 && typeof cb!='function')
...
tcoll.prototype.count = function (query, cb) {
	var self = this;
	if (typeof query=='function' && cb==null) {
		cb = query;
		query = null;
	}
	if (query==null || Object.keys(query).length==0) {
		this._tq.add(function (cb) {
			cb(null, Object.keys(self._store).length);
		},false,cb);
	} else
...
tcoll.prototype.stats = function (cb) {
	var self = this;
	this._tq.add(function (cb) {
		cb(null, {count:Object.keys(self._store).length});
	},false,cb);
}
...
function applySet(obj, $set, tdb) {
	for(var k in $set){
		...
		var v = $set[k];
		if (v && typeof v=='object' && !(v instanceof tdb.ObjectID || v instanceof Date || v instanceof tdb.Code) ) {
			if (!t[k]){
				t[k]={};
			}
			applySet(t[k],v,tdb);
...
function simplifyKey(key) {
	var k = key;
	if (key.toJSON){
		// FIXME the result of toJSON doesn't necessarially have a toString method
		k = key.toJSON();
	}
...
tcoll.prototype.save = function (doc, opts, cb) {
	var self = this;
	cb = (typeof doc=='function' && doc) || (typeof opts=='function' && opts) || cb || function(){};
...
tcoll.prototype.__find = function (query, fields, skip, limit, sort_, order, hint, arFields, cb) {
	...
	var qtLen = Object.keys(qt).length;
	var pi = [];
	var io = {};
	...
	if (pi.length>0) {
		...
		if (self._idx[sort]) {
			var values = self._idx[sort].all();
			values.forEach(function(_id) {
				range.push(self._store[_id].pos);
			});
			if (order==-1){
				range.reverse();
			...
			range = Object.keys(self._store).map(function(k){ return self._store[k].pos; });
		}
	}
	...
	if (sort==null && (qtLen==0 || qt._args.length==0)) {
	...
	matcher = new Function('obj', 'return '+(arrayMatch?qt.native3():qt.native()));
...
tcoll.prototype.group = function (keys, condition, initial, reduce, finalize, command, options, callback) {
	...
	with (options.scope || {}) {
		try {
			if (typeof keys=='function') keys = eval('(' + keys + ')');
		...
		async.doUntil(
			function (cb) {
				c.nextObject(safe.sure(cb, function (_doc) {
					...
					if (!doc) return cb();
					var keys2 = keys;
					if (typeof keys=='function') keys2 = keys(doc);
