	...
	$where: function () {
		...
		this.native = this.native3 = function (obj) {
			...
			if (typeof v=='function') return '(' + v + ').call(obj)';
				...
				if (typeof v.code=='function') return '(function () { with (' + JSON.stringify(v.scope) + ') return (' + v.code + ').call(obj); })()';
...
for(k in ops){
	ops[k].prototype.fields = function () {
		var f = {};
		var _args = this._args;
		this._args.forEach(function (a) {
			if (a.op=="f")
				f[a.f]=1;
			else if (a.op!="v") {
				var s = a.fields();
				for(var k in s) f[k]=1;
			}
		})
		return f;
	}
...
function value(v) {
	...
	this.native3 = this.native = function () {
		return (typeof this.v=='function') ? this.v.toString() : JSON.stringify(this.v);
	}
}
...
function stree(query,ctx) {
	...
	} else if (query && typeof query=='object' && !(query instanceof tdb.ObjectID || query instanceof Date || query instanceof tdb.Code)) {
		for(var k in query){
			var v = query[k];
			var op = ops[k];
			var n = null;
			if (op!=null) {
				...
				if (typeof v=='object' && !(v instanceof tdb.ObjectID || v instanceof Date || query instanceof tdb.Code)) {
