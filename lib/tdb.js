...
tdb.prototype.close = function (forceClose, cb) {
	...
	async.forEach(Object.keys(self._cols).map(function(n){return self._cols[n];}), function (c, cb) {
		c._stop(cb)
...
tdb.prototype._nameCheck = function (cname) {
	...
	if (cname.length==0)
		return new Error("collection names cannot be empty");
	if (cname.indexOf("$")!=-1)
		return new Error("collection names must not contain '$'");

	var di = cname.indexOf(".");
	if (di==0 || di==cname.length-1){
		return new Error("collection names must not start or end with '.'");
	}
...
tdb.prototype.collectionNames = function (opts, cb) {
	...
	fs.readdir(self._path, safe.sure(cb,function(files) {
		...
		files = _.union(files, Object.keys(self._cols))
...
tdb.prototype.collections = function (cb) {
	...
	self.collectionNames({namesOnly:1},safe.sure(cb, function (names) {
		async.forEach(names, function (cname, cb) {
			...
			cb(null, Object.keys(self._cols).map(function(n){return self._cols[n];}));
		}))
	}))
}
